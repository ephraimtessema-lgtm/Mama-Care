      ...
      <div className="bg-white border-b sticky top-14 z-10">
        <div className="max-w-5xl mx-auto px-4 py-4">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <h1 className="text-xl font-bold text-gray-900">📚 Pregnancy Library</h1>
              <p className="text-xs text-gray-400">Trusted articles from medical experts</p>
            </div>
          </div>
          <div className="relative mb-3">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
            <Input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search articles..." className="pl-9 rounded-full text-sm" />
          </div>
          <div className="flex gap-2 overflow-x-auto pb-1">
            <Button size="sm" onClick={() => setFilterCat("all")}
              className={`rounded-full text-xs flex-shrink-0 ${filterCat === "all" ? "bg-rose-500 hover:bg-rose-600 text-white" : "border border-rose-200 text-rose-600 bg-white hover:bg-rose-50"}`}>
              All Topics
            </Button>
            {Object.entries(CATEGORY_CONFIG).map(([k, v]) => (
              <Button key={k} size="sm" onClick={() => setFilterCat(k)}
                className={`rounded-full text-xs flex-shrink-0 ${filterCat === k ? "bg-rose-500 hover:bg-rose-600 text-white" : "border border-rose-200 text-rose-600 bg-white hover:bg-rose-50"}`}>
                {v.label}
              </Button>
            ))}
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map(article => (
            <div key={article.id}
              ...
              {!article.image_url && (
                <div className="w-full h-36 bg-gradient-to-br from-rose-100 to-pink-100 flex items-center justify-center text-4xl">📋</div>
              )}
              <div className="p-4">
                ...
                <h3 className="font-bold text-gray-900 text-sm mb-1 line-clamp-2">{article.title}</h3>
        ...
        <DialogContent className="max-w-2xl rounded-2xl max-h-[85vh] overflow-y-auto">
          {selected && (
            <>
              ...
              <div className="prose prose-sm max-w-none text-gray-700 whitespace-pre-wrap leading-relaxed">{selected.content}</div>
